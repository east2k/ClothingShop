...
const errorPopup = document.querySelector(".error-popup-box");
const errorPopupText = document.querySelector(".error-popup-box span");
const errorXButton = document.querySelector(".popup-x-button");

confirmButton.addEventListener('click', () => {
    if (storedCartList.length < 1) {
        errorPopup.classList.add("active");
        errorPopupText.innerHTML = "Cart is Empty";
    } else if (confirmFullName.value === "" || confirmPhone.value === "") {
        errorPopup.classList.add("active");
        errorPopupText.innerHTML = "Name or Phone number is Empty";
        console.log("Fill the name");
        console.log("Fill the phonenumber");
    } else if (confirmFullName.value !== currentProfileLogged.fullName) {
        errorPopup.classList.add("active");
        errorPopupText.innerHTML = "Make sure Name is same as Registered";
...
errorXButton.addEventListener('click', () => {
...
// Purchase Done
const purchaseDone = document.querySelector(".purchase-done");
const pageBlocker = document.querySelector(".page-blocker");

finalConfirmYes.addEventListener('click', () => {
    for (let i = 0; i < storedCartList.length; i++) {
        currentProfileLogged.accountStats.totalPurchases.push(storedCartList[i]);
    }
    currentProfileLogged.accountStats.totalCashSpent.push(total);
    let max,
        priceList = [];
    for (let i = 0; i < currentProfileLogged.accountStats.totalPurchases.length; i++) {
        priceList.push(currentProfileLogged.accountStats.totalPurchases[i].price);
    }
    max = Math.max.apply("Math", priceList);
    currentProfileLogged.accountStats.highestItemBought.push(max);
    currentProfileLogged["userOrders"] = [];
    purchaseDone.classList.add("active");
